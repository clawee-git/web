...
- **Version badge:** `index.html` carries a `<span class="ver">v0.1.62</span>` next to the Install heading. Bump it on every `clawee` release (and bump `style.css?v=N` if styles change so the edge cache refreshes).
